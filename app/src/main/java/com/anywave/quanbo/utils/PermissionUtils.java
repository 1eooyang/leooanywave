...
public class PermissionUtils {
    ...
    public static final int CODE_ACCESS_WIFI_STATE = 0;
    public static final int CODE_ACCESS_NETWORK_STATE = 1;
    ...
    public static void requestMultiPermissions(final Activity activity, PermissionGrant grant) {
        ...
        if (permissionsList.size() > 0) {
            ...
            Log.d(TAG, "should open those permission");
    ...
    public static void requestAnyPermission(final Activity activity, View mLayout, final int requestCode, PermissionGrant grant) {
        if (activity == null && mLayout == null) {
            return;
        }
        ...
        if (ActivityCompat.checkSelfPermission(activity, requestPermission) != PackageManager.PERMISSION_GRANTED) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(activity, requestPermission)) {
                Snackbar snackbar = Snackbar.make(mLayout, "提示：全播缺少权限不能正常连接Wifi,请点击\"授权\".否则将无法正常收看视频!",
                        ...
                        }).setActionTextColor(activity.getResources().getColor(R.color.color_tick_hot));
                snackbar.getView().setBackgroundColor(activity.getResources().getColor(R.color.nomal_color_bg));
                snackbar.show();
            } else {
                ActivityCompat.requestPermissions(activity, new String[]{requestPermission}, requestCode);
            }
    ...
    public static void requestPermissionsResult(final Activity activity,final int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults, PermissionGrant permissionGrant) {
        ...
        if (grantResults.length == 1 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
         ...
         /*   Snackbar snackbar = Snackbar.make(view, "Qpop缺少权限不能正常连接Wifi,请点击\"授权\".否则将无法正常收看视频!",
                    Snackbar.LENGTH_INDEFINITE)
                    .setAction("OK", new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
